    ...
    const l1 = _
      ...
      .then((squares) => {
        ...
        squares.forEach((square) => {
          ...
          const name = `Gent ${square.name.nl
            .split('/')[0]
            .trim()
            .replace(/ /g, '+')
            .replace(/ç/g, 'c')
      ...
      .catch(console.error);
    const t1 = _
      .fetch('https://datatank.stad.gent/4/infrastructuur/publieksanitair.geojson')
      .then(data => data.json())
      .then((json) => {
        this.toilets.push(...json.coordinates.map(el => ({ lat: el[1], long: el[0] })));
      });
    const t2 = _
      .fetch('https://datatank.stad.gent/4/toerisme/mobieletoilettengentsefeesten.geojson')
      .then(data => data.json())
      .then((json) => {
        this.toilets.push(...json.coordinates.map(el => ({ lat: el[1], long: el[0] })));
      });
